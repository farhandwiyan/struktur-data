...
boolean IsUnerLeft (bintree3 P) {
   ...
   if (!IsEmptyTree(P)) {
      if (right(P) == NIL && left(P) != NIL) {
         return True;
      } else { // jika kanan tidak kosong atau kiri kosong
         return False;
      }
   } else {
      return False;
...
boolean IsUnerRight (bintree3 P) {
   ...
   if (!IsEmptyTree(P)) {
      if (right(P) != NIL && left(P) == NIL) { 
         return True;
      } else { // jika kanan kosong atau kiri tidak kosong
         return False;
      }
   } else {
      return False;
...
/*function linearPosfix(P:bintree3) -> list1 {menghasilkan list node dari P terurut posfix kiri,kanan,akar}*/ 
...
/* Helper untuk copy level ke List */
void copyLevelToList(bintree3 P, int N, List1 *L) {
    if (!IsEmptyTree(P)) {
        if (N == 1) {
            InsertVLast(L, info(P));
        } else {
            copyLevelToList(left(P), N - 1, L);
            copyLevelToList(right(P), N - 1, L);
        }
    }
}
